...
int kondo_init_custom(KondoRef ki, int baud, int vid, int pid, int interface, UINT index)  // 20151028 iwaishi
{
  int res;
	...
	// open usb device
	if (res=ftdi_usb_open_desc_index(&ki->ftdic, vid, pid, NULL, NULL, index) < 0)  // 20151028 iwaishi
	  kondo_ftdi_error(ki);
...
int kondo_trx(KondoRef ki, int out_bytes, int in_bytes)
{
	...
	if ((i = kondo_purge(ki)) < 0) {
	  printf("kondo_trx: kondo_purge error %d\n", i);
	  return i;
	}
	if ((i = kondo_write(ki, out_bytes)) < 0) {
	  printf("kondo_trx: kondo_write error %d\n", i);
	  return i;
	}
...
int kondo_get_options(KondoRef ki)
{
  //	assert(ki);
	...
	if ((i = kondo_trx(ki, 10, 5)) < 0) {
	  printf("get_options return %d ", i);
	  for (i=0;i<5;i++) printf("%x ", ki->swap[i]);
	  printf("\n");
	  return i;
	}
...
int kondo_play_motion(KondoRef ki, UINT num, long timeout)
{
	...
	if ((ret = kondo_trx(ki, 19, 4)) < 0) {
	  printf("kondo_play_motion first error %d :",ret );
	  for (i=0; i<19; i++) printf(" %x", ki->swap[i]);
	  printf("\n");
	  return ret;
	}

	// verify response checksum
	if (ret != 4 || ki->swap[18] != chk)
	  kondo_error(ki, "Bad response trying to stop EEPROM.");

	// (2) call motion script -------------------------------------------------
	// You have to compute the motion address (base + num * size) and call it.
	//	mot_addr = (RCB4_MOT_SIZE * (num - 1)) + 0x380 + RCB4_MOT_SIZE;
	...
	if ((ret = kondo_trx(ki, 7, 4)) < 0) {
	  printf("kondo_play_motion second step error %d", ret);
	  for (i=0; i<7; i++) printf(" %x", ki->swap[i]);
	  printf("\n");
	  return ret;
	}

	// verify response (ack)
	if (ret != 4 || ki->swap[2] != RCB4_ACK_BYTE)
	...
	if ((ret = kondo_trx(ki, 9, 4)) < 0) {
	  printf("third step");
	  for (i=0; i<9; i++) printf(" %x", ki->swap[i]);
	  printf("\n");
	  return ret;
	}
	...
	while ((tv.tv_sec < end.tv_sec) || (tv.tv_usec < end.tv_usec)) {
	  ...
	  if ((ret = kondo_get_options(ki)) < 0) {
	    printf("kondo_play_motion loop timeout get_options ret=%d\n",ret);
	    printf("opt[0]=%x, opt[1]=%x\n", ki->opt[0],ki->opt[1]);
	    return ret;
	  }
...
int kondo_send_ics_pos(KondoRef ki, UCHAR servos[5], UINT frame)
{
	...
	ki->swap[2] = servos[0];	// changed order servos
	ki->swap[3] = servos[1];
	ki->swap[4] = servos[2];
	ki->swap[5] = servos[3];
	ki->swap[6] = servos[4];
